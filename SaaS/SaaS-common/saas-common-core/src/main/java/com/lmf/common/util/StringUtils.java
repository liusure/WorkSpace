...
public class StringUtils extends org.apache.commons.lang3.StringUtils {
	...
	public static String filterParameters(String url, List<String> filters) {

		int index = url.indexOf("?");

		if (index > 0) {
			String preUrl = url.substring(0, index);
			String afterurl = url.substring(index + 1);
			index = afterurl.indexOf("#");
			List<String> paramList = new ArrayList<>();
			String parameterstr = "";
			String anchor = "";
			if (index >= 0) {
				parameterstr = afterurl.substring(0, index);
				anchor = afterurl.substring(index);
			} else {
				parameterstr = afterurl;
			}
			...
			for (String param : params) {
				...
				if (kv.length == 2) {
					if (!filters.contains(kv[0])) {
						paramList.add(param);
					}
				}
			}
			parameterstr = Collections3.convertToString(paramList, "&");
			return preUrl.concat("?").concat(parameterstr).concat(anchor);
	...
	public static String addParameters(String url, List<KeyValue> parameters) {
		int index = url.indexOf("#");
		String preUrl = url;
		String anchor = "";
		if (index > 0) {
		...
		parameters.forEach((kv) -> {
			addParams.append(kv.getKey()).append("=").append(kv.getValue()).append("&");
		});

		if (preUrl.indexOf("?") > 0) {
			if (preUrl.endsWith("&") || preUrl.endsWith("?")) {
				preUrl += addParams.toString();
			} else {
				preUrl += "&" + addParams.toString();
			}
		} else {
		...
		// }
		return preUrl + anchor;
	}

	public static Map<String, String> parseParameters(String url) {
		String parameterStr = url;
		int index = url.indexOf("?");
		int index2 = url.indexOf("#");
		if (index >= 0) {
			parameterStr = url.substring(index + 1, index2 > 0 ? index2 : url.length());
		}
		Map<String, String> map = new HashMap<>();
		String[] paras = parameterStr.split("&");
